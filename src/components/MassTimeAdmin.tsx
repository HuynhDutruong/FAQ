...
import {
  MassTime, getDioceses, getByDiocese, createMass, updateMass, deleteMass,
  refreshDioceseMeta, importFromJson, parseTimes, removeAccents
} from '@/lib/massTimes';
...
export default function MassTimeAdmin() {
  ...
  const loadDioceses = useCallback(() => {
    getDioceses().then(setDioceses).catch(e => setError(e.message));
  }, []);

  const loadRows = useCallback((d: string) => {
    if (!d) { setRows([]); return; }
    setLoading(true);
    getByDiocese(d).then(setRows).catch(e => setError(e.message)).finally(() => setLoading(false));
  }, []);

  useEffect(loadDioceses, [loadDioceses]);
  useEffect(() => loadRows(diocese), [diocese, loadRows]);
  ...
  const handleSave = () => {
    if (!draft) return;
    if (!draft.parish.trim() || !draft.diocese.trim()) {
      setError('Bắt buộc nhập Tên nhà thờ và Giáo phận.');
      return;
    }
    run('save', async () => {
      const { id, ...data } = draft;
  ...
  const shown = term
    ...
    : rows;
  const total = dioceses.reduce((s, d) => s + d.count, 0);

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      {/* Thanh công cụ */}
      <div style={{ ...card, padding: '20px', display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'center' }}>
        <select value={diocese} onChange={e => setDiocese(e.target.value)} style={{ ...input, width: 'auto', minWidth: '260px' }}>
          <option value="">-- Chọn Giáo phận ({total} nhà thờ) --</option>
        ...
        <input placeholder="Tìm nhà thờ / địa chỉ..." value={q} onChange={e => setQ(e.target.value)}
          disabled={!diocese} style={{ ...input, width: 'auto', flex: '1 1 200px' }} />

        <button onClick={() => setDraft({ ...EMPTY, diocese })} style={btn('#059669')}>
          <Plus size={16} /> Thêm
        </button>
        ...
        <button onClick={() => run('meta', async () => { await refreshDioceseMeta(); loadDioceses(); })}
          disabled={!!busy} style={btn('#6B7280')} title="Đếm lại số nhà thờ mỗi giáo phận">
          <RefreshCw size={16} />
        </button>
      </div>
      ...
      {!diocese ? (
        <div style={{ ...card, padding: '40px', textAlign: 'center', color: '#6B7280' }}>
          {total === 0
            ...
            : 'Chọn một Giáo phận để xem và chỉnh sửa danh sách nhà thờ.'}
        </div>
      ) : loading ? (
          ...
          <div key={m.id} style={{ ...card, padding: '16px', display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: '0.85rem', color: '#6B7280', marginTop: '2px' }}>
                {[m.deanery, m.address].filter(Boolean).join(' — ') || '(chưa có địa chỉ)'}
              </div>
